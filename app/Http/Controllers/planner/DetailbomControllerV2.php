...
class DetailbomControllerV2 extends Controller
{
    ...
    public function bomDetail(String $id_bom)
    {
        ...
        $unsubmittedDetailBom = Detailbomv2::where('id_boms', $id_bom)
            ->where('submitted', false)
            ->orderBy('id_materialbom', 'asc')->get();

        $submittedDetailBom = Detailbomv2::where('id_boms', $id_bom)
            ->where('submitted', true)
            ->get();
        // dd($submittedDetailBom);
    ...
    public function storematerial(Request $request): RedirectResponse
    {
        ...
        $material->id_warehouse = $request->get('id_warehouse');
        ...
        $material->comparison = $request->get('comparison');
        $material->composite = $request->get('composite');

        $tolerance = (strtolower($request->uom_material) === 'kg') ? 0.025 : 0;

        $usagematerial = ($request->comparison * $request->composite) * (1 + $tolerance);
    ...
    {
        $detailbomItem = Detailbomv2::where('id_materialbom', $id_materialbom)
            ->where('id_boms', $id_bom)
            ->first();
    ...
    {
        $detailBom = Detailbomv2::where('id_materialbom', $id_materialbom)
    ...
    {
        $detailboms = Detailbomv2::where('id_boms', $id_bom)->get();
    ...
    {
        $notifMaterial = Detailbomv2::where('id_boms', $idBom)
            ->where('db_status', 0)
    ...
    public function updateStatusbom($id_bom)
    {
        ...
        $detailboms = Detailbomv2::where('id_boms', $idBom)->get();
        ...
        Bomv2::where('id_bom', $id_bom)->update(['status_bom' => $statusBom]);
    }
    ...
    public function submit(Request $request)
    {
        ...
        Detailbomv2::where('id_boms', $idBom)->update(['submitted' => 1]);
    ...
    public function restoreMaterial(Request $request, $id_materialbom, $id_bom)
    {
        ...
        $detailBom = Detailbomv2::where('id_materialbom', $id_materialbom)->where('id_boms', $id_bom)->first();
