...
@section('main', 'show')
    ...
    <div class="card-body">
      <form>
        ...
        <div class="col text-right">
          <a href="{{ route('bom_v2-addmaterial', ['id_bom' => $dataBom->id_bom]) }}" class="btn btn-primary"><i class="mr-2 fa fa-plus" aria-hidden="true"></i>Add New Material</a>
        </div>
        <br>
  ...
  <!-- Modal SUBMIT -->
  <form id="submitForm" action="{{ route('bom_v2.submit') }}" method="POST">
    @csrf
  ...
  <script>
    $(document).ready(function() {
      ...
      $('#deleteModal').on('show.bs.modal', function(event) {
        ...
        const actionUrl = "{{ route('bommaterial_v2.delete', ['id_materialbom' => ':id_materialbom', 'id_bom' => $dataBom->id_bom]) }}";
      ...
      $('#deleteForm').on('submit', function(event) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            window.location.href = "{{ route('bom_v2.detailbom', ['id_bom' => $dataBom->id_bom]) }}";
          },
          error: function(error) {
  ...
  $('.confirmRestoreButton').on('click', function() {
    ...
    var url = "{{ route('bommaterial_v2.restore', ['id_materialbom' => ':id_materialbom', 'id_bom' => ':id_bom']) }}";
